...
public:
    ...
    int change(int amount,vector<int>& coins) {
        ...
        memo.resize(amount+1,vector<int>(n+1,INT_MAX));
        
        //unordered_Sets:
        //---------------
        //return ccRec(amount,n);
        
        //Un-Ordered Sets: With limited supply:
        // -----------------------------------
        // return ccRecLtd(amount,n);
        ...
        //Bottom Up For iterate all approach:
        //Only one parameter changing hence could be solved using 1D dp.
        
        //Bottom Up: For Choose Not Choose strategy with unltd supplies.
...
//         return dp[amount];
    ...
    int ccRecLtd(int amt,int n)
    {   
        if(n == 0 && amt)return 0;
        if(memo[amt][n] != INT_MAX)
                return memo[amt][n];
        if(!amt)return 1;
        
        int inc=0,exc=0;
        
        exc = ccRecLtd(amt,n-1);
        if(amt>=c[n-1])
            inc = ccRecLtd(amt-c[n-1],n-1);
        
        return memo[amt][n] = exc + inc;
    }
};