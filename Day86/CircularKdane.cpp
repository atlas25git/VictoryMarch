...
class Solution{
    ...
    int circularSubarraySum(int arr[], int n){
        ...
        //we get two sums
        //once the subarray sum with normal kadane,
        ...
        for(int i=0;i<n;i++)
            arr[i]<0?count+=1:count+=0,
            maxx=max(maxx,arr[i]),
            sum+=arr[i],sumF=max(sum,sumF),
            (sum<0?sum=0:sum+=0);
        ...
        //min subArray
        for(int i=0;i<n;i++)arr[i]=-arr[i];//to get the minimum subArray sum in arr
